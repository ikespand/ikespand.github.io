...
For my (academic) research work, I was keeping it simple i.e. maintaining a CSV which was appending by a Python function for each experiments. So far, it was working quite nicely (for only single user which is me). But, obvious downside was that I start having nested folders for the artifacts such as models, graphs, latent features etc. Also, opening the big csv file also looks daunting. Therefore, I decide to try out MLFlow for my future experimentation and docker is obviour choice. There are many great repositories exist (like [this](https://github.com/Toumash/mlflow-docker) and [this one](https://github.com/at-gmbh/docker-mlflow-server)). For my use-case, I'd like to use:
...
Therefore, I created yet another [GitHub repository](https://github.com/ikespand/docker-mlflow-minio) which includes MLFlow and MinIO along with the SQLite. 

## Steps to running the setup locally or remote server
One can setup the MLFlow server on a single machine using this approach. It can either a local machine from which you will run ML experiments or a remote machine which is simple used for tracking. In my case, it is local-machine.
...
4. Spin of the docker for MinIO and MLFlow with SQLite by `docker-compose up`. Use Powershell on Windows as volume mounting can have problems with `git-bash`.
5. As a result, you should able to see [localhost:5000](http://localhost:5000/) for our MLFlow server while [localhost:9001]() for MLFlow and [localhost:5000](http://localhost:5000/) for MinIO. 
...
7. At this point, you can observe that `mlflow_data` and `minio_data` folders have created in the repository.  You can also configure the location of these folder by modifying `docker-compose.yml`. Here, you overall setup will look like as following:
![img](/_posts/assets-2022-09-09-blog-post-1/dockerstartup.png)


### How it works?
- In `docker-compose` services, we first build the image for mlflow which is pretty simple. We use the official python image and install few dependencies. I explicitly froze the version because I had problem with the newer version of the libraries. We then use this build image and define all the key environment variables which MLFlow expects. The source of these still remains `.env` file.
...
![img](/_posts/assets-2022-09-09-blog-post-1/bashprofile.png)
...
- After running the script, you can see this new experiment in the dashborad and browse through parameters and artifacts like following:
  ![img](/_posts/assets-2022-09-09-blog-post-1/mlflowoutput.png)
## Next steps?
